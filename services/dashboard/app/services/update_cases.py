...
def get_national_cases(date: str, interval: str):
    if interval not in ["1", "7"]:
        return 0
    # update national cases
    return update_national(date, interval)


def get_region_data(date: str, city: str, interval: str, ratio: bool = False):
    # load taiwan geo data
    taiwan_admin = load_taiwan_admin()
    
    data = {}
    for region in taiwan_admin[city]:
        region_data = update_region(date, city, region, interval, ratio)
        if ratio:
...
def get_city_data(date: str, interval: str, ratio: bool = False):
    # load taiwan geo data
    taiwan_admin = load_taiwan_admin()
    
    data = {}
    for city in taiwan_admin.keys():
        city_data = update_city(date, city, interval, ratio)
        if ratio:
